...
async function processNote(noteId: string) {
  ...
  if (!contentText) {
    ...
    await db.note.update({ where: { id: noteId }, data: { status: 'failed', aiProcessingError: 'No text available for AI processing', aiProcessedAt: new Date() } });
    return;
  }
  ...
  try {
    ...
    try {
      ...
      else errors.push('Summary validation failed');
    } catch (e) { errors.push('Summary parse failed'); }
  } catch (e) { errors.push('AI summary error'); }
  ...
  try {
    ...
    try {
      ...
      else errors.push('Flashcards validation failed');
    } catch (e) { errors.push('Flashcards parse failed'); }
  } catch (e) { errors.push('AI flashcards error'); }
  ...
  try {
    ...
    try {
      ...
      else errors.push('MCQs validation failed');
    } catch (e) { errors.push('MCQs parse failed'); }
  } catch (e) { errors.push('AI mcq error'); }

  const hadValidResults = !!summaryData || flashcards.length > 0 || mcqs.length > 0;
  ...
  try {
    await db.$transaction(async (tx) => {
      ...
      if (hadValidResults) {
        await tx.note.update({ where: { id: noteId }, data: { status: 'active', aiProcessingError: null, aiProcessedAt: new Date() } });
      } else {
        await tx.note.update({ where: { id: noteId }, data: { status: 'failed', aiProcessingError: errors.join('; ') || 'No valid results', aiProcessedAt: new Date() } });
      }
    });
  } catch (dbErr) {
    console.error('Worker: DB save failed', dbErr);
    try { await db.note.update({ where: { id: noteId }, data: { status: 'failed', aiProcessingError: 'DB save failed', aiProcessedAt: new Date() } }); } catch {}
  }
}
...
async function pollLoop() {
  ...
  while (true) {
    try {
      ...
      if (!queued) { await sleep(5000); continue; }
