...
function OverviewTab({ onTabChange }: { onTabChange: (tab: string) => void }) {
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <StatCard title="Total Users" value={stats.totalUsers} icon={Users} subtitle="Registered" color="bg-emerald-500" />
        <StatCard title="Total Notes" value={stats.totalNotes} icon={FileText} subtitle="Uploaded" color="bg-teal-500" />
      ...
      {data?.stats?.recentReports && data.stats.recentReports.length > 0 && (
        ...
        >
          <Card className="border-0 shadow-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <AlertTriangle className="size-4 text-orange-500" /> Recent Reports
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {data.stats.recentReports.map((report: { id: string; targetType: string; reason: string; status: string; createdAt: string; reporter: { name: string; avatarUrl?: string } }) => (
                  <div key={report.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                    <Avatar className="size-7">
                      {report.reporter.avatarUrl && <AvatarImage src={report.reporter.avatarUrl} />}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        Reported by {report.reporter.name} · {report.targetType}
                      </p>
                    </div>
...
function UsersTab() {
  ...
  const roleBadgeColor = (role: string) => {
    switch (role) {
      case 'super_admin': return 'bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300';
      case 'admin': return 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300';
      case 'moderator': return 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300';
      case 'contributor': return 'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300';
      case 'verified_student': return 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300';
...
function NotesTab() {
  ...
  const notes: {
    id: string; title: string; fileType: string; status: string;
    downloadCount: number; viewCount: number; avgRating: number;
    createdAt: string;
  ...
  const statusBadge = (status: string) => {
    switch (status) {
      ...
      case 'flagged': return 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300';
      case 'removed': return 'bg-destructive/10 text-destructive';
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">No notes match the current filter.</p>
        </motion.div>
      ) : (
        <Card className="border-0 shadow-sm">
          <Table>
                ...
                <TableRow key={n.id}>
                  <TableCell>
                    <div className="min-w-0">
                      ...
                      {n.subject && (
                        <p className="text-[10px] text-muted-foreground">{n.subject.name}</p>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                      <span className="flex items-center gap-0.5"><Download className="size-3" />{n.downloadCount}</span>
                      <span className="flex items-center gap-0.5"><Eye className="size-3" />{n.viewCount}</span>
                      <span className="flex items-center gap-0.5"><Star className="size-3 text-amber-500" />{n.avgRating > 0 ? n.avgRating.toFixed(1) : '—'}</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      <Avatar className="size-5">
                        {n.uploader.avatarUrl && <AvatarImage src={n.uploader.avatarUrl} />}
                      ...
                      <span className="text-xs text-muted-foreground truncate max-w-[80px]">{n.uploader.name}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-right">
      ...
      {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {statusFilter === 'pending' ? 'No pending reports to review.' : 'No reports match the current filter.'}
          </p>
        </motion.div>
      ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge className={`text-[10px] ${statusBadgeColor(report.status)}`}>
                          {report.status}
                        </Badge>
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Add the first college to get started.</p>
        </motion.div>
      ) : (
        <Card className="border-0 shadow-sm">
          <Table>
                ...
                <TableRow key={college.id}>
                  <TableCell>
                    <div className="min-w-0">
                      ...
                      {college.shortName && (
                        <p className="text-[10px] text-muted-foreground">{college.shortName}</p>
                      )}
                    </div>
                  </TableCell>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setNewCollege((p) => ({ ...p, shortName: e.target.value }))}
